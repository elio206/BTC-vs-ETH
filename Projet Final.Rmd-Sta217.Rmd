...
```{r, Step 6}
###################################################################################
##################ETAPE 6: ESTIMATION DES PARAMETRES DES VE SELON GPD  :
# Charger les bibliothèques
library(extRemes)

# Charger vos données Bitcoin
btc_data <- read.csv("C:/Users/ebouserhal/Desktop/cv/STA217/BTC-USD.csv")
# Charger vos données Ethereum
eth_data <- read.csv("C:/Users/ebouserhal/Desktop/cv/STA217/ETH-USD.csv")
# Sélectionner les colonnes de prix de clôture
btc_close <- btc_data$Close
eth_close <- eth_data$Close
# Estimer les paramètres de la distribution GPD pour Bitcoin
btc_fit <- fevd(btc_close, type = "GP", threshold = quantile(btc_close, probs = 0.95))
# Estimer les paramètres de la distribution GPD pour Ethereum
eth_fit <- fevd(eth_close, type = "GP", threshold = quantile(eth_close, probs = 0.95))
# Afficher les résultats
summary(btc_fit)
summary(eth_fit)

####### RESULTATS :
#Les résultats que vous avez obtenus après l'estimation des paramètres de la distribution GPD (Generalized Pareto Distribution) pour les séries temporelles des prix de clôture du Bitcoin (BTC) et d'Ethereum (ETH) sont les suivants :

# Pour Bitcoin (BTC) :
  
#  Estimation du paramètre d'échelle (scale) : 5203.8155487
# Estimation du paramètre de forme (shape) : -0.4122298
# Erreur standard pour le paramètre d'échelle : 652.35878956
# Erreur standard pour le paramètre de forme : 0.08479281
# Matrice de covariance estimée pour les paramètres :
 #  [scale, scale] : 425571.99031
# [shape, shape] : 0.00718982
 # [scale, shape] : -50.18063171
 # AIC (Critère d'information d'Akaike) : 1687.9
 # BIC (Critère d'information bayésien) : 1692.944

 # Pour Ethereum (ETH) :

# Estimation du paramètre d'échelle (scale) : 682.983713
 #    Estimation du paramètre de forme (shape) : -0.620846
 #     Erreur standard pour le paramètre d'échelle : 78.61890324
# Erreur standard pour le paramètre de forme : 0.08265517
 # Matrice de covariance estimée pour les paramètres :
# [scale, scale] : 6180.931947
 #[shape, shape] : 0.006831877
# [scale, shape] : -6.337226734
# AIC (Critère d'information d'Akaike) : 1275.657
...
```{r, step 6-ETH}
############################# Quel est le modes de la serie ETH ??
##########################################################################
##########################################################################
####################### MODELE eGARCH(1,1)
